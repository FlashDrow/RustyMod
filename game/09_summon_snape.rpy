label summon_snape:
    ...
    menu snape_menu:
        ...
        "Задание: пыльца плотоядного фикуса" if (((phx_pot == 2) or (phx_pot == 3)) and (phx_ingr2 == 0)):
            m "Дружище, ты ведь преподаватель зельеварения?"
            $ s_sprite = "03_hp/10_snape_main/23.png"
            sna "Самый лучший преподаватель зельеварения"
            m "отлично"
            $ s_sprite = "03_hp/10_snape_main/snape_01.png"
            sna "а что?"
            m "у тебя случайно не завалялась пыльца плотоядного фикуса?"
            $ s_sprite = "03_hp/10_snape_main/snape_11.png"
            sna "Чего?"
            m "...."
            m "ну... всего лишь простенький ингридиент..."
            $ s_sprite = "03_hp/10_snape_main/snape_30.png"
            sna "Простенький?!{w} Плотоядный фикус цветет раз в 50 лет! Да за его пыльцу можно вагон шлюх снять..."
            $ s_sprite = "03_hp/10_snape_main/snape_01.png"
            sna "а зачем тебе она?"
            m "..."
            m "это будет намного лучше вагона шлюх, поверь мне"
            $ s_sprite = "03_hp/10_snape_main/snape_04.png"
            sna "...."
            m "Это нужно для нашего общего дела, гермиона будет буквально лежать у тебя в ногах"
        ...
        "-Спросить про стенгазету-" if paper_level == 2:
            m "Привет, Северус !"
            m "Меня не покидает желание открыть свою газетную редакцию. Подскажешь, что к чему ?"
            $ s_sprite = "03_hp/10_snape_main/snape_02.png"
            sna "Джинни, есть всего две вещи в мире, которые интересуют меня меньше газет."
            $ s_sprite = "03_hp/10_snape_main/snape_22.png"
            sna "Но говорить о них мне хочется еще меньше, чем о газетах. Есть намного более приятные занятия."
            m "Как по твоему, я смогу делать газету в одиночку ?"
            $ s_sprite = "03_hp/10_snape_main/snape_20.png"
            sna "Далась тебе эта газета. Давай я лучше расскажу, сколько очков и за что я дал недавно одной маленькой шлю... то есть ученице."
            m "Пожалуйста, не отвлекайся, для меня это важно !"
            $ s_sprite = "03_hp/10_snape_main/24.png"
            sna "Ну ладно, ладно. Ты же пишешь свои дурацкие отчеты ? Ну и газету сделать сможешь."
            $ s_sprite = "03_hp/10_snape_main/snape_02.png"
            sna "Укажи внизу мелким шрифтом, что в редакции четыре анонимных студента, чтобы списать все промахи на них. А сам будь как бы ни при чём."
            $ s_sprite = "03_hp/10_snape_main/snape_03.png"
            sna "Только помни, что чем меньше людей тебя тут видят, тем меньше шанс раскрыться. Не стоит приглашать к себе посторонних."
            g4 "Минутку ! Если я буду делать всё сам, не выходя из кабинета, то как же газета окажется на стене главного холла ?"
            $ s_sprite = "03_hp/10_snape_main/snape_02.png"
            sna "Да нет ничего проще. У Альбуса на столе есть магическая печать. С ее помощью любой проштампованный лист попадает прямо на стенд."
            sna "Так что с этим проблем не предвидится."
            $ s_sprite = "03_hp/10_snape_main/snape_17.png"
            sna "Пожалуй на этом все, твои распросы пора уже патентовать как снотворное зелье."
            $ paper_level = 3
            stop music fadeout 1.0
            $ s_sprite = "03_hp/10_snape_main/snape_01.png"
            $ menu_x = 0.5 #Menu is moved to the left side. (Default menu_x = 0.5)
        ...
        "-Спросить про Хагрида-" if quest_hagrid == 1:
            m "кхм, снейп, дружище, а как ты думаешь, за счет чего я выгляжу как этот ваш Думалдым?"
            $ s_sprite = "03_hp/10_snape_main/snape_03.png"
            sna "Дамблдор"
            m "ну да"
            ...
            sna "я уже говорил, я не могу это понять, это какая-то абсолютно другая магия"
            $ s_sprite = "03_hp/10_snape_main/snape_03.png"
            m "хм"
            m "а у вас здесь есть какие-то способы.... стать похожим на кого-то?"
            $ s_sprite = "03_hp/10_snape_main/snape_02.png"
            sna "конечно"
            m "какие?"
            $ s_sprite = "03_hp/10_snape_main/snape_10.png"
            sna "кто-то может делать это сам с рождения, чаще только в собаку, медведя, еще какую-то живность"
            $ s_sprite = "03_hp/10_snape_main/snape_03.png"
            m "...."
            m "я говорю о человеке"
            $ s_sprite = "03_hp/10_snape_main/snape_10.png"
            sna "я тебя понял"
            $ s_sprite = "03_hp/10_snape_main/snape_02.png"
            sna "есть несколько артефактов, которые успешно лежат под сотней замков"
            g4 "*черт*"
            $ s_sprite = "03_hp/10_snape_main/snape_10.png"
            sna "самый просто способ - зелье превращение"
            $ s_sprite = "03_hp/10_snape_main/snape_02.png"
            sna "ингридиенты совсем не сложные, главное - частичка того, в кого хочешь превратиться"
            m "отлично"
            $ s_sprite = "03_hp/10_snape_main/snape_04.png"
            sna "....."
            ...
            sna "а зачем тебе?"
            g4 "эм...."
            m "просто было интересно"
            $ s_sprite = "03_hp/10_snape_main/snape_03.png"
            sna "хорошо если так, тебе лучше оставаться здесь, как бы ты не выглядел"
            m "...."
            $ quest_hagrid = 2
            m "*похоже что он не поможет*"
            g4 "*черт*"
            jump snape_menu

        "-Спросить про зелье для Чжоу Чанг-" if chimi_event == 8:
            m "Ты даешь этой Чанг свое лекарство?"
            $ s_sprite = "03_hp/10_snape_main/24.png"
            ...
            m "ладно"
            sna "вот и хорошо"
            g4 "*нужно придумать чем подменить это его зелье*"
            $ s_sprite = "03_hp/10_snape_main/snape_03.png"
            $ chimi_event = 9
            jump snape_menu
        ...
        "Мне пришло какое-то стремное письмо..." if daphne_event == 1:
            jump mizulina_snape

        "- Поговорить -" if daytime and not chitchated_with_snape: # sfmax - friendship with Snape maxed out.
            $ chitchated_with_snape = True #Prevents you from chitchating more then once a day. Turns back to False every night and every day.
            $ menu_x = 0.5 #Menu is moved to the left side. (Default menu_x = 0.5)
            if (day > 30) and (whoring > 12) and (devil == 0):
                jump snape_kniga
            else:
                jump snape_chitchat
        ...
        "-\"Отвиснуть.\"-" if sfmax and ((dap_level >= 11) and (daphne_event == 11)):
            $ menu_x = 0.5 #Menu is moved to the left side. (Default menu_x = 0.5)
            jump snape_dates

        "-\"Отвиснуть.\"-" if not daytime and not sfmax: # Turns TRUE when friendship with Snape been maxed out.
            if one_of_ten == 10:
                call not_today from _call_not_today #Snape says: "I am busy tonight."
...
#                call not_today #Snape says: "I am busy tonight."
                ...
                call not_today from _call_not_today_1 #Snape says: "I am busy tonight."
            else:
                pass
                $ menu_x = 0.5 #Menu is moved to the left side. (Default menu_x = 0.5)
                jump snape_dates
...
label snape_dates:  ### HANGING WITH SNAPE ###
    ...
    if (daphne_event >= 6) and (dap_publ == 0):
        call quiditch from _call_quiditch


    elif (dap_level >= 11) and (daphne_event == 11):
        call snape_kram from _call_snape_kram

    elif daphne_event == 100:
        call snape_afterdaph from _call_snape_afterdaph
